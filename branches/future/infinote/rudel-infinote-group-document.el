...
(defmethod rudel-infinote/request
  ...
  (with-tag-attrs (user user number) xml
    (let* ((operation (car (xml-tag-children xml))) ;; TODO are multiple operations possible?
	   (type      (xml-tag-name operation)))
      (rudel-infinote-dispatch ;; TODO
       this type
       (list user operation)
       "rudel-infinote/request/")))
  nil)
