...
;; reverse operation.  It is part of the torrent metafile
;; specification at <http://bittorrent.org/protocol.html>.


;;; History:
;;
;; 0.2 - Initial version in Rudel
;;
;; 0.1 - Initial version
...
(defun bencode--pair (pair)
  ""
  (destructuring-bind (first second) pair
    (concat (bencode first)
	    (bencode second))))
...
(defun bencode (obj)
  ...
  (cond
   ...
   ((and (listp obj) (eq (car obj) 'dict))
    (concat "d"
	    (mapconcat #'bencode--pair (cdr obj) "")
	    "e"))

   ((hash-table-p obj)
    (concat "d"
	    (apply #'concat (maphash #'bencode--pair obj))
	    "e"))
   ...
   (t
    (signal 'wrong-type-argument (type-of obj))))
  )
  ...
  (cond
   ...
   ((looking-at "d")
    ...
    (let (dict key)
      (while (setq item (bdecode-buffer))
	(if key
	    (setq dict (cons (list key item) dict)
		  key nil)
	  (unless (stringp item)
