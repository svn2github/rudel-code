...
`rudel-tls-make-process'."
  ...
  ;; TLS handshake.
  (message "tls-start-tls: switching to tls handshake filter")
...
(defun rudel-tls-wait-init (process data)
  ...
  (let ((client-data)
    ...
    (rudel-loop-lines data line
      (when (and (not (string-match "This method.*" line))
		 (not (string-match "Resolving.*" line))
		 (not (string-match "Connecting.*" line))
		 (not (string-match "- Simple Client Mode.*" line)))
